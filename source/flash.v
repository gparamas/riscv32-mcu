// *============================================================================================== 
// *
// *   MX25L3233F.v - 32M-BIT CMOS Serial Flash Memory
// *
// *           COPYRIGHT 2017 Macronix International Co., Ltd.
// *----------------------------------------------------------------------------------------------
// * Environment  : Cadence NC-Verilog
// * Reference Doc: MX25L3233F REV.1.6,MAR.10,2017
// * Creation Date: @(#)$Date: 2017/04/06 02:20:55 $
// * Version      : @(#)$Revision: 1.15 $
// * Description  : There is only one module in this file
// *                module MX25L3233F->behavior model for the 32M-Bit flash
// *----------------------------------------------------------------------------------------------
...
// *============================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
        ...
        `define Vtclqv_1xio 6  //30pf:8ns, 15pf:6ns
...
// *============================================================================================== 
    ...
    parameter   tSHQZ           = `LVR ? 10 : 8,     // CS High to SO Float Time [ns]
                ...
                tCLQV_1XIO      = `Vtclqv_1xio, // Clock Low to Output Valid for 1XIO
                ...
                tCE             = 10_000,      // unit is ms instead of ns  
                tPP             = 330_000,      // Program time
                tW              = 40_000_000,       // Write Status time
                tWSR            = 1_000_000,     // Write Security Register time
    ...
    specify
        specparam   tSCLK   = 7.5,    // Clock Cycle Time [ns]
                    ...
                    tCH     = 3.4,      // Clock High Time (min) [ns]
                    tCL     = 3.4,      // Clock Low  Time (min) [ns]
                    ...
                    tTSCLK  = 9.6,    // Clock Cycle Time for 2XI/O READ + 4 Dummy
                    fTSCLK  = 104,    // Clock Frequence for 2XI/O READ + 4 Dummy
                    tTSCLK2 = 7.5,    // Clock Cycle Time for 2XI/O READ + 8 Dummy
                    fTSCLK2 = 133,    // Clock Frequence for 2XI/O READ + 8 Dummy
                    tQSCLK  = 9.6,    // Clock Cycle Time for 4XI/O READ + 6 Dummy
                    fQSCLK  = 104,    // Clock Frequence for 4XI/O READ + 6 Dummy
                    tQSCLK2 = 7.5,   // Clock Cycle Time for 4XI/O READ + 10 Dummy
                    fQSCLK2 = 133,  // Clock Frequence for 4XI/O READ + 10 Dummy
                    tFSCLK  = 7.5,    // Clock Cycle Time for Fast READ + 8 Dummy
                    fFSCLK  = 133,    // Clock Frequence for Fast  READ + 8 Dummy
                    tFQSCLK = 7.5,    // Clock Cycle Time for Fast 4XO READ + 8 Dummy
                    fFQSCLK = 133,    // Clock Frequence for Fast 4XO READ + 8 Dummy
                    tFDSCLK = 7.5,    // Clock Cycle Time for Fast 2XO READ + 8 Dummy
                    fFDSCLK = 133,    // Clock Frequence for Fast 2XO READ + 8 Dummy
...
// *==============================================================================================
    ...
    always @( SIO3_Reg or SIO2_Reg or SIO1_Reg or SIO0_Reg ) begin
        if ( SIO3_OUT_EN && WP_OUT_EN && SO_OUT_EN && SI_OUT_EN ) begin
            ...
            SIO1_Out_Reg <= #tCLQV_1XIO SIO1_Reg;
        end
    end
...
// *============================================================================================== 
                    ...
                    RDP, RES:
                        begin
                            if ( ( !WIP || During_Susp_Wait) && Chip_EN && ~HPM_RD ) begin
                                ...
                                RES_Mode = 1'b1;
                                Read_SHSL = 1'b1;
                                if ( CS_INT == 1'b1 && ISCLK == 1'b0 &&
                                   ((Bit >= 38 && !ENQUAD) || (Bit >=35 && ENQUAD)) ) begin
                                    tRES1_Chk = 1'b0;
                                    tRES2_Chk = 1'b1;
                                    DP_Mode = 1'b0;
                                end
                                else if ( CS_INT == 1'b1 && ISCLK == 1'b1 && Bit >=39 ) begin
                                    tRES1_Chk = 1'b0;
                                    tRES2_Chk = 1'b1;
                                    DP_Mode = 1'b0;
                                end
    ...
    always @ ( RST_Event ) begin
        ...
        else if ( (WRSR_Mode||WRSR2_Mode) || WRSCUR_Mode || WR_WPSEL_Mode || PP_4XIO_Mode || PP_1XIO_Mode ) begin
            #tRCP;
        end
...
// *========================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
    /*  Description: suspend area function                                  */
    /*  INPUT: address                                                      */
    /*----------------------------------------------------------------------*/
    function address_in_susp;
        input [A_MSB:0] Address;
        begin
            if ( Secur_Mode == 1'b0 ) begin
                 if ( Address[A_MSB:19] == Bank2 && EPSUSP == 1'b1 ) begin
                        address_in_susp = 1'b1 ;
                        $display ( $time," Failed programing,address is in program or erase" );
                 end
                 else begin
                        address_in_susp = 1'b0 ;
                 end
            end
            else if ( Secur_Mode == 1'b1 ) begin
               address_in_susp = 1'b0 ;
...
// *==============================================================================================
    ...
    wire FastRD_1XIO_Chk_W;
    assign FastRD_1XIO_Chk_W = FastRD_1XIO_Chk;
    ...
    specify
        ...
        $period( posedge  SCLK &&& FastRD_1XIO_Chk_W , tFSCLK ); // SCLK _/~ ->_/~
        $period( posedge  SCLK &&& FastRD_2XIO_Chk_W , tFDSCLK ); // SCLK _/~ ->_/~
        $period( posedge  SCLK &&& FastRD_4XIO_Chk_W , tFQSCLK ); // SCLK _/~ ->_/~
    ...
    always @ ( posedge SCLK ) begin
        ...
        if ( $realtime - T_SCLK_P < tQSCLK2 && Read_4XIO_Chk && (CR[6]==1'b1) && $realtime > 0 && ~CS )
            $fwrite (AC_Check_File, "Clock Frequence for 4XI/O instruction fQSCLK =%f Mhz, fQSCLK timing violation at %f \n", fQSCLK2, $realtime );
        //fFSCLK
        if ( $realtime - T_SCLK_P < tFSCLK && FastRD_1XIO_Chk && $realtime > 0 && ~CS )
            $fwrite (AC_Check_File, "Clock Frequence for Fast 1XO instruction fFSCLK =%f Mhz, fFSCLK timing violation at %f \n", fFSCLK, $realtime );
        //fFDSCLK
        if ( $realtime - T_SCLK_P < tFDSCLK && FastRD_2XIO_Chk && $realtime > 0 && ~CS )
            $fwrite (AC_Check_File, "Clock Frequence for Fast 2XO instruction fFDSCLK =%f Mhz, fFDSCLK timing violation at %f \n", fFDSCLK, $realtime );
        //fFQSCLK
        if ( $realtime - T_SCLK_P < tFQSCLK && FastRD_4XIO_Chk && $realtime > 0 && ~CS )
